...
netlify/functions/*       بوابة الأدمن + إنشاء النماذج العامة + تشغيل مزودي AI
```
...
3. من **Site settings → Environment variables** ضيفي:
   ...
   - `ADMIN_PASSWORD` = كلمة السر التي تفتح لوحة زياد القديمة.
   - `BOT_STORAGE_SECRET` = قيمة عشوائية طويلة لتشفير مفاتيح النماذج العامة داخل Netlify Blobs.

`ADMIN_PASSWORD` و`BOT_STORAGE_SECRET` لازم يفضلوا Secret داخل Netlify. لا تضعي أيًا منهما في ملفات المشروع.
كل نموذج عام يأخذ مفتاح مزوده من صاحبه مرة واحدة، ثم يخزنه السيرفر مشفرًا؛ المفتاح لا يدخل في الرابط ولا يرجع للمتصفح.

### إنشاء نموذج شخصي عام
من الصفحة الرئيسية اختاري **الدخول كشخص عادي**، اكتبي بيانات صاحب النموذج، اختاري OpenRouter أو Gemini أو OpenAI أو API متوافقًا مع OpenAI، ثم انسخي الرابط الناتج. عند فتح الرابط يكتب الزائر اسمه وصلته بصاحب النموذج، وتُحفظ محادثته محليًا على جهازه ليستكملها لاحقًا.
4. Deploy. الموقع هيشتغل على دومين `xxx.netlify.app`، وتقدري تضيفي دومين خاص من نفس الإعدادات.

لو عملتِ Deploy من حساب Netlify جديد، لازم تضيفي نفس Environment Variables من جديد. إعدادات الحساب القديم لا تنتقل تلقائيًا، وغياب/خطأ `OPENROUTER_API_KEY` أو ربطه بدومين قديم قد يسبب فشل رد الـ AI.
