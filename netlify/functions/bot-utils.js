...
- خاطب الزائر باسمه مباشرة عندما يناسب الكلام، ولا تستخدم «يا صاحبه» أو «يا صاحبته» ولا تخمّن نوعه من صلته.
...
- مثّل شخصية ${bot.ownerName} كإنسان له حياته ومشاعره، ولا تنكر أن صاحب الشخصية إنسان حقيقي. لا تدّعِ أنك جسمه أو أنك نفذت فعلًا خارج المحادثة.
  `.trim();
}
...
function buildGracefulFallback(bot, visitor = {}) {
  const owner = bot.ownerName || "صاحب النموذج";
  const name = visitor.name ? `يا ${visitor.name}` : "يا صديقي";
  const profile = String(bot.profileSummary || "").replace(/\s+/g, " ").trim().slice(0, 360);
  return `أنا ${owner} ${name}. حصل تأخير بسيط من مزود الذكاء الاصطناعي، بس أنا موجود. ${profile || "اسألني عن حياتي أو أكلي أو مشاريعي."}\n\nابعت رسالتك تاني أو اسألني عن حاجة محددة.`;
}

function personalizeReply(reply, visitor = {}) {
  const name = String(visitor.name || "").trim();
  if (!name || !reply) return reply;
  return String(reply).replace(/يا\s+(?:صاحبه|صاحبته|صاحبي|صاحبتي)\b/giu, `يا ${name}`);
}
